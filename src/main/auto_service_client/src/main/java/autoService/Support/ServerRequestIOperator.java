...
public class ServerRequestIOperator
{
	...
	{
		String url = baseUrl + "clients" + "/" + phone;
		String result = get(url);
		if (result != null)
		{
			if (!result.equals(""))
	...
	{
		String url = baseUrl + "time" + "/" + value + "/" + duration;
	...
	public static String postService(JSONObject service) throws IOException
	{
		String url = baseUrl + "serviceCreate";
		return post(url, service);
	}
